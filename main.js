...
const PERVOMAYSKAYA = {lat: 53.893818, lng:27.570962};
const NEMIGA = {lat: 53.905947, lng: 27.554066};
const FRUNZENSCAYA = {lat: 53.905225, lng: 27.539205};
const LENINA = {lat: 53.892892, lng: 27.547887};
const OKTYABORSKAYA = {lat: 53.902131, lng: 27.562246};
const POBEDI = {lat: 53.909543, lng: 27.576308};

const mapOptions = {zoom: 14, center: HUMAN_POSITION };
const select = document.getElementById('station-selector');
const route = document.getElementById('show-route');
...
function initMap() {
    ...
    const human = new google.maps.Marker({
    ...
    const pervomayskayaMarker = new google.maps.Marker({
    ...
    const nemigaMarker = new google.maps.Marker({
        position: NEMIGA,
        animation: google.maps.Animation.DROP,
        map: map,
        title: 'Немига',
    ...
    const frunzenscayaMarker = new google.maps.Marker({
        position: FRUNZENSCAYA,
        animation: google.maps.Animation.DROP,
        map: map,
        title: 'Фрунзенская',
    });  
    const leninaMarker = new google.maps.Marker({
        position: LENINA,
        animation: google.maps.Animation.DROP,
        map: map,
        title: 'Площадь Ленина',
    });
    const oktyaborskayaMarker = new google.maps.Marker({
        position: OKTYABORSKAYA,
        animation: google.maps.Animation.DROP,
        map: map,
        title: 'Октяборьская',
    });    
    const pobediMarker = new google.maps.Marker({
        position: POBEDI,
        animation: google.maps.Animation.DROP,
        map: map,
        title: 'Площадь победы',
    });  
    const markers = [pervomayskayaMarker,nemigaMarker,frunzenscayaMarker,
    ...
    select.addEventListener('change', function () {
        map.setCenter(eval(select.value.toUpperCase())); 
        eval(select.value + 'Marker').setAnimation(google.maps.Animation.BOUNCE);
        const stopAnimation = function(){
            return eval(select.value.toLowerCase()  + 'Marker').setAnimation(null)
        }
        ...
        directionsService.route(
            {
              ...
              destination: eval(select.value.toUpperCase()),
              travelMode: 'WALKING',
    ...
    const clickMarker = function(marker){
